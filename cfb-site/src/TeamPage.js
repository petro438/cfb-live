...
function TeamPage() {
  ...
  const API_URL = 'https://cfbapi-production.up.railway.app'; // Fixed to production URL
  ...
  const loadTeamData = React.useCallback(async () => {
    ...
    try {
      ...
      // Extract the teams array from the rankings response
      const rankings = rankingsData?.teams || rankingsData || [];
      
      // Convert string numbers to actual numbers for team data
      const processedTeam = {
        ...team,
      ...
      // Convert string numbers for stats
